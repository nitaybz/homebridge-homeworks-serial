...
const processLine = function (line) {
	...
	const message = {
		...
		address: splittedLine[1].replace(/[[\]]/g, ''),
	...
	const rawDeviceFound = this.homeworksDevices.find(device => {
		if (device.rawStatus)
			return Object.values(device.rawStatus).find(value => value.replace(/\s/g, '') === line.replace(/\s/g, ''))
		return false
	})
			...
			case 'KBH':
				...
				if (!deviceFound)
					deviceFound = this.homeworksDevices.find(device => device.type === 'windowCovering' && device.address === message.address && device.pressType === keypadDictionary[message.command] &&
						( device.openButtonId == message.state || device.closeButtonId == message.state || device.stopButtonId == message.state || device.louverButtonId == message.state ))
				if (!deviceFound)
	...
	if (rawDeviceFound) {
		updateRawDevice.bind(this)(rawDeviceFound, line)
	} else if (keypadFound) {
...
function updateRawDevice(device, line) {
	const commandKey = Object.keys(device.rawStatus).find(key => device.rawStatus[key].replace(/\s/g, '') === line.replace(/\s/g, ''))

	switch (device.type) {
		case 'switch':
		case 'outlet':
			setTimeout(() => {
				device.updateHomeKit({On: commandKey === 'on'})
			}, 500)
			break
		case 'windowCovering':
			if (commandKey === 'louver')
				device.toggleLouver()
			else 
				device.updatePositionCommand(commandKey)
			break
	}
}
...
function updateDevice(device, message) {
	switch (device.type) {
		...
		case 'button':
			setTimeout(() => {
				device.updateHomeKit({ ProgrammableSwitchEvent: message.command === 'KBP' ? 0 : message.command === 'KBDT' ? 1 : 2 })
			}, 500)
			break
		case 'windowCovering':
			if (message.state == device.louverButtonId)
				device.toggleLouver()
			else {
				var position = message.state == device.openButtonId ? 'open' : message.state == device.closeButtonId ? 'close' : 'stop'
				device.updatePositionCommand(position)
			}
			break
	}
