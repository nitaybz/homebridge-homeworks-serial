...
const processLine = function (line) {
	...
	const rawDeviceFound = this.homeworksDevices.find(device => {
		if (device.rawStatus)
			return Object.values(device.rawStatus).find(value => value === line)
		return false
	})
			...
			case 'KBH':
				...
				if (!deviceFound)
					deviceFound = this.homeworksDevices.find(device => device.type === 'windowCovering' && device.address === message.address && device.pressType === keypadDictionary[message.command] &&
						( device.openButtonId == message.state || device.closeButtonId == message.state || device.stopButtonId == message.state ))
				if (!deviceFound)
...
function updateRawDevice(device, line) {
	const commandKey = Object.keys(device.rawStatus).find(key => device.rawStatus[key] === line)
